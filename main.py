...
comparison_matrices = {
    'goal': {
        'matrix': np.array([[1, 5 / 7, 1 / 5, 5 / 9, 1 / 8], [7 / 5, 1, 5 / 9, 5 / 7, 4 / 5], [5, 9 / 5, 1, 7 / 3, 4 / 3],[9 / 5, 7 / 5, 3 / 7, 1, 9 / 7], [8, 5 / 4, 3 / 4, 7 / 9, 1]]),
        'subcriteria': {
            'cost': {
                'matrix': np.array([[1, 1 / 7, 1 / 8], [7, 1, 1 / 3], [8, 3, 1]]),
                'subcriteria': {
                    'purchase_price': {
                        'matrix': np.array([[1, 5 / 7, 9 / 4, 5 / 4], [7 / 5, 1, 7 / 6, 6 / 7], [4 / 9, 6 / 7, 1, 2 / 3],[4 / 5, 7 / 6, 3 / 2, 1]])
                    },
                    'fuel_cost': {
                        'matrix': np.array([[1, 3 / 7, 5 / 9, 1 / 2], [7 / 3, 1, 5 / 8, 5 / 8], [9 / 5, 8 / 5, 1, 1 / 2],[2, 8 / 5, 2, 1]])
                    },
                    'maintenance_cost': {
                        'matrix': np.array([[1, 5 / 7, 3 / 4, 9 / 5], [7 / 5, 1, 1 / 2, 5 / 6], [4 / 3, 2, 1, 2 / 3],[5 / 9, 6 / 5, 3 / 2, 1]])
                    },
                }
            },
            'capacity': {
                ...
                'subcriteria': {
                    'trunk_size': {
                        'matrix': np.array([[1, 5 / 6, 3 / 2, 2 / 5], [6 / 5, 1, 9 / 5, 5 / 7], [2 / 3, 5 / 9, 1, 1],[5 / 2, 7 / 5, 1, 1]])
                    },
                    'passenger_capacity': {
                        'matrix': np.array([[1, 1 / 9, 1 / 9, 8 / 3], [9, 1, 3 / 2, 9], [9, 2 / 3, 1, 9], [3 / 8, 1 / 9, 1 / 9, 1]])
                    },
                }
            },
            'design': {
                'matrix': np.array([[1, 9, 9, 9], [1 / 9, 1, 1 / 5, 8 / 9], [1 / 9, 5, 1, 9 / 7], [1 / 9, 9 / 8, 7 / 9, 1]]),
            },
            'safety': {
                'matrix': np.array([[1, 5 / 2, 9, 7], [2 / 5, 1, 9, 4], [1 / 9, 1 / 9, 1, 1 / 5], [1 / 7, 1 / 4, 5, 1]]),
            },
            'warranty': {
                'matrix': np.array([[1, 1 / 9, 3 / 4, 5 / 7], [9, 1, 9, 9], [4 / 3, 1 / 9, 1, 2], [7 / 5, 1 / 9, 1 / 2, 1]]),
            }
        }
    }
}
...
def calculate_ranking(comparison_matrix):
    if 'subcriteria' in comparison_matrix:
        ...
        return np.matmul(np.stack(geometric_rankings, axis=-1), comparison_matrix['normalized_geometric']), np.matmul(np.stack(eigen_rankings ,axis=-1), comparison_matrix['normalized_eigen'])
    else:
